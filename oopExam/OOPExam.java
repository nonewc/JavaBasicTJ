...
/*
		객체지향의 핵심은 '실제 세계는 객체로 이루어져 있고, 
		발생하는 모든 사건들은 객체들간의 상호작용이다.'
		실제 세계를 컴퓨터 속에 옮겨 놓은 것과 같은 가상세계를 구현하고 
		이를 통해 많은 비용과 시간을 절약할 수 있다.
	
	객체지향 언어의 특징 (재사용성, 유지보수, 중복코드 제거)
	
		1. 코드의 재사용성이 높다.
		- 새로운 프로그램을 만들 때 기존에 만들어 놓은 코드를 그대로 가져다가 쓸 수 있다.
	...
	클래스와 객체
		...
		클래스를 한 번만 제대로 만들기만 하면 매번 객체를 어떻게 만들까 고민할 필요가 없다.
	...
	객체와 인스턴스
		...
		인스턴스는 클라스와의 연계된 표현이 필요할 경우 사용된다.
			...
			예) 책상은 객체다(o)	책상은 목수클래스로 만든 인스턴스이다. (o)
			책상은 인스턴스다 (x)	책상은 목수클래스로 만든 객체이다. (x)
	...
	# 객체 배열
		...
		예를 들면 게시물 객체들을 배열로 모두 담아 처리하면 수월하게 게시판 관리를 할 수 있다.
		엄밀히 말하면 실제 객체들이 배열에 저장되는 것이 아니라
...
*/
		...
		public static void main(String[] args) {
			...
			Tv[] tvArr = new Tv[3]; // 'Tv클래스로 만든 객체들'만 담을 수 있는 tvArr 배열을 3개짜리로 생성 
