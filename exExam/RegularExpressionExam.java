...
/*	Regular Expression 정규식, 정규표현식
	...
	많은 양의 텍스트 데이터에서 원하는 데이터를 쉽게 뽑아낼 수도 있고,
	어떤 데이터가 형식에 맞는지 체크할 수 있다.
	예를 들면 html에서 전화번호나 이메일 주소만 추출하거나 입력한 비밀 번호가
	숫자와 영문자의 조합으로 되어있는지 등을 쉽게 확인할 수 있다.
...
*/
	...
	public static void main(String[] args) {
		...
		//Pattern p = Pattern.compile("c[0-9]"); // c로 시작하고
		...
		Pattern p = Pattern.compile("c[a-z]*"); // c로 시작하는 소문자 영단어
		//문자열 배열에 담긴 문자열들 중에서 지정한 정규식과 일치하는 문자열을 찾아서
		// 출력 Pattern은 정규식을 정의하고 Matcher는 비교하는 역할을 한다.
		// 1. 정규식을 매개변수로 Pattern클래스의 static 메서드인 pattern.compile()
		//을 호출하여 Pattern 객체를 얻는다.
